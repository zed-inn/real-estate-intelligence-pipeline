...
import { info, validation, PropertySchema } from "@/gen/property/property_pb";
...
function writeToFile(
  schema: ReturnType<typeof createPropertySchema>,
  filename: string,
) {
  ...
  appendFileSync(
    absFilename,
    `\n\nexport const PropertySchema = z.object({${Object.keys(schema)}})`,
  );

  appendFileSync(
    absFilename,
    `\n\nexport const PropertySearchableSchema = z.object({${Object.entries(
      schema,
    )
...
const FILENAME = "property_zod_schema.ts";

writeToFile(createPropertySchema(), FILENAME);
